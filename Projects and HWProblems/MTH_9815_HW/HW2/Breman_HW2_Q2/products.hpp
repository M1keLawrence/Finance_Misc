...
public:
  // Future ctor
  Future(string _productId, string _underlying, double _contractSize, date _maturityDate)
  ...
  // Overload << operator to print the future
  friend ostream& operator<<(ostream &output, const Future &future)
  {
    output << future.underlying << " " << future.contractSize << " " << future.maturityDate;
    return output;
  }

private:
  string underlying;      // underlying product
  double contractSize;    // contract size
  date maturityDate;      // maturity date
};

/**
 * Modeling of a EuroDollar Future Product
 */
class EuroDollarFuture : public Future
{
public:
  EuroDollarFuture(string productId, date maturityDate)
    : Future(productId, "EuroDollar", 1000000.0, maturityDate)
  {
  }
...
{
public:
  BondFuture(string productId, string underlyingBond, date maturityDate)
    : Future(productId, underlyingBond, 100000.0, maturityDate)
  {
  }
